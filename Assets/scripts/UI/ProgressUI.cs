...
public class ProgressUI : MonoBehaviour
{
    ...
    public void changeOfTurn(int id)
    {
        ...
        string[] s = { "t", "‰Ä", "H", "“~" };
        nameText.text = p.name;nowText.text = s[GameMain.gameMain.nowSeason.getID()] + ":" + (GameMain.gameMain.nowRound + 1).ToString("0") + "ƒ^[ƒ“–Ú";
        academicText.text = "Šw—Í " + p.academic.getValue().ToString("0");
        apperanceText.text = "—eŽp " + p.appearance.getValue().ToString("0");
        luckText.text = "‰^ " + p.luck;
        string[] a = { "ƒoƒXƒP•”", "‘tŠy•”", "ƒoƒCƒg" };
        affiliationText.text = "Š‘® " + a[p.activity.getID()];
    }
